...
var gName = "g_cc_small";
var deleteGraphName = "g_cc_small"
...
if (process.env.VCAP_SERVICES) {
  ...
  db_vertexes = cloudant.db.use("db_cf_cc_vertexes_ref");
  db_edges = cloudant.db.use("db_cf_cc_edges_ref");
  db_issuelist = cloudant.db.use("db_cf_cc_issue_list");
...
var loadVertexData=function loadVertexData(){
  ...
  db_vertexes.find({selector:{type:'vertex', processVertex:true}}).then(function(cldVArr){
    ...
    console.log("All Vertexes processed now:grphVArr: len: " + JSON.stringify(grphVArr));
    ...
    return Q.all([
      ...
      console.log('#################cldVArr: len %s###########', JSON.stringify(cldVArr));
  }).catch(function(err) {
